...
foreach ($ind_match in $ind_arr) {
    ...
    # DEBUG STATEMENTS
    echo "[DEBUG] [Selected Monitor]"
    echo $monitor_info[$ind_match - 1]
    echo $monitor_info[$ind_match + 1]
    ...
    # DEBUG STATEMENTS
    echo "[DEBUG] [Selected Monitor Tuple Info]"
    echo $mon_tuple
}
...
foreach ($set_cmd in $set_VR_res_cmd_arr) {
    echo "[DEBUG] [APPLYING 1080p SET COMMAND]"
    echo $set_cmd
...
foreach ($reset_cmd in $reset_res_cmd_arr) {
    echo "[DEBUG] [APPLYING RESET RESOLUTION COMMAND]"
    echo $reset_cmd
    Invoke-Expression $reset_cmd
}

# DEBUG TIMER
echo "SCRIPT TERMINATING IN 30 SECONDS, COPY DEBUG STATEMENTS"
Start-Sleep -s 30